...
To do that, we can annotate the `print` rule with the tag `[klabel(print)]` (?) (to be able to refer to it), and recompile with `kompile exp.k --transition "print"`. (This feature requires the OCAML backend.)
